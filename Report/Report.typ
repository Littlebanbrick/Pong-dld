...
#h(2em)本项目旨在实现打乒乓（Pong）小游戏，并在其基础上进行优化并添加额外内容，增加可玩性的同时提高游玩舒适度。

=== 基础功能

#h(2em)该游戏为双人对战小游戏（也可单人与 AI 对战），每人操作一个球拍进行接球。左右球拍分别使用键盘 W/S 和 ↑/↓ 键控制上下移动。球遇到上下边界与球拍会反弹，未接到球者判负。系统实时记录双方得分，先得 11 分者获胜，同时屏幕上显示 GAME OVER。

=== 额外功能 1——难度选择

#h(2em)可利用 SW[2:3] 选择游戏难度。游戏分为四个难度：Easy、Hard、Master、Auto。其中 Easy、Hard、Master 的球速逐级递增，而 Auto 较为特殊——初始速度与 Easy 一致，但每次击球后球速变为原来的 1.1 倍，使比赛更加紧张刺激。

同时，选择的难度会在四个七段数码管上显示。

=== 额外功能 2——道具

#h(2em)比赛每隔一段时间会在双方球拍附近生成神秘小道具，接触到道具者的球拍会短时间内延长（延长的部分显示为灰色），使接球更加容易。

=== 额外功能 3——随机化发球角度

#h(2em)每次发球的角度都会在一定范围内随机化，增加开球的不可预测性，让双方选手开局即进入状态。

=== 额外功能 4——蜂鸣器与拖尾

#h(2em)击球、失分、游戏结束均配有不同频率的蜂鸣器音效；同时球的移动带有拖尾视觉效果，增强速度感。

=== 额外功能 5——PS/2 接口

#h(2em)本游戏支持通过 PS/2 接口连接 USB 键盘进行操作，有效改善了双人对战的操作体验。

=== 额外功能 6——AI 对手

#h(2em)提供一套较为简单的 AI 行为逻辑，实现对右挡板的自动控制。该 AI 通过追踪球的 Y 坐标位置，配合死区（60px）和随机更新延迟实现自然的追踪效果。当球远离时，AI 会缓慢回中，为下次防守做准备。
...
#h(2em)用于显示当前游戏难度，分为 Easy、Hard、Master、Auto 四种。
...
#h(2em)== defines.vh——全局宏定义头文件

#h(2em)用于定义全项目通用的参数，方便统一管理，主要宏定义如下：
#align(center,table(
  columns: (auto, auto, auto),
  align: left,
...
#h(2em)该模块功能主要分为以下几部分：状态机的设计（前文已述）、game_tick 分频控制、球运动与碰撞（前文已述）、反弹角度计算（前文已述）、宽挡板道具、边沿检测。
...
#h(2em)`vga_render` 是"画师"模块，根据当前扫描到的像素坐标和各游戏对象坐标，逐像素计算颜色输出。
...
#h(2em)`game_logic` 是位于 `ai_paddle` 之上的模块，负责游戏综合逻辑。
...
#h(2em)激活三类事件（击球、得分、结束）的脉冲，检查 `buzz` 输出是否正常。
...
#h(2em)- 模拟 PS/2 总线时序（时钟约 16.7kHz，数据帧格式）。
