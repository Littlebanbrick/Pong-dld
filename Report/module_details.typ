// ============================================================================
// module_details.typ — Pong 项目各模块详细技术说明
// ============================================================================
...
#set page(
  ...
  header: context [
    #text(size: 12pt, fill: gray.darken(25%))[Pong 模块详细说明]
    #h(1fr)
...
#align(center + horizon)[
  ...
  #v(2em)
  #text(size: 16pt)[版本：1.0]
  #v(0.5em)
  #text(size: 16pt)[#datetime.today().display("[year]-[month]-[day]")]
]
...
#h(2em)`defines.vh` 是全项目的"参数中枢"，使用 Verilog 宏（define）定义了所有关键常量，目的在于：
...
== game_logic.v — 游戏逻辑核心状态机

#h(2em)这是整个项目最核心的模块，实现了一个有限状态机（FSM）控制游戏流程，并包含球的物理运动和碰撞检测。
...
#h(2em)使用 19 位计数器 `tick_counter`，从 0 计数到 `tick_threshold`。`tick_threshold` 根据难度设定不同值：
- Easy: 419583 → 约 60Hz
- Hard: 209791 → 约 120Hz
...
#h(2em)- *速度表示*：`ball_dx`（水平速度）和 `ball_dy`（垂直速度）均为 11 位有符号数，支持负方向
- *位置更新*：`next_x_s = ball_x + ball_dx`，`next_y_s = ball_y + ball_dy`，使用有符号运算避免溢出

*碰撞检测逻辑*：

1. #strong[上下边界反弹]：当 `next_y_s ≤ 0` 或 `next_y_s ≥ 464` 时，`ball_dy` 取反
2. #strong[左挡板碰撞]：当球右缘进入左挡板区域 (`next_x_s ≤ 30 && next_x_s + 8 ≥ 20`) 且 Y 坐标与挡板重叠时触发
3. #strong[右挡板碰撞]：类似，检测球左缘与右挡板 (`next_x_s + 8 ≥ 610 && next_x_s ≤ 620`) 的重叠
4. #strong[出界得分]：若球未碰挡板且 `next_x_s ≤ 0`（右方得分）或 `next_x_s + 8 ≥ 640`（左方得分）

=== 反弹角度计算

#h(2em)碰撞时根据球心与挡板中心的偏移量确定反弹角度：
...
#h(2em)当 `pw_hit_left` 或 `pw_hit_right` 脉冲到来时，`wide_timer` 设为 300 个 game_tick（约5秒），期间挡板上下各扩展5像素（灰色显示）。

=== 难度与挡板速度

#h(2em)挡板速度也随难度缩放，但步长×频率保持"等效速度"合理：
...
#h(2em)`start_pause` 和 `soft_reset` 信号需要边沿检测（`start_pause_d` 延迟一拍后比较），确保一次按键只触发一次状态转换，而不是连续触发。

---

== vga_render.v — VGA 图像渲染

#h(2em)`vga_render` 是"画师"模块，根据当前扫描到的像素坐标和各种游戏对象坐标，逐像素计算颜色输出。

=== 球的拖尾效果

#h(2em)使用三级移位寄存器保存球的前三帧位置：
```
game_tick 有效时：
...
#h(2em)每个数字由 16 行 × 8 列的位图（bitmap）表示，存储在 `digit_bitmap` 寄存器数组中。0~9 的位图是硬编码的点阵字体 ROM。
...
#h(2em)"GAME OVER" 由 9 个字符（含空格）组成，每个字符也是 8×16 点阵字体。通过 `TEXT_SCALE` 宏（=2）将每个字符放大到 16×32 像素。
...
#h(2em)`vgac.v` 是 VGA 信号的"行场同步发生器"，产生标准的 640×480 \@60Hz VGA 时序。

=== 计数器

#h(2em)- `h_count`：水平计数器，0~799（共800个时钟周期，含消隐区）
...
#h(2em)- 行同步 `hs`：`h_count > 95` 时为高（同步脉宽96像素）
- 场同步 `vs`：`v_count > 1` 时为高（同步脉宽2行）
- 显示使能 `rdn`（低有效）：当 `143 < h_count < 783` 且 `34 < v_count < 515` 时有效（即 640×480 可见区域）

=== 坐标输出

#h(2em)- `row_addr = v_count - 35`（减去消隐区偏移，得到 0~479）
- `col_addr = h_count - 144`（减去消隐区偏移，得到 0~639）

=== 颜色输出

#h(2em)12位输入 `d_in = {B[3:0], G[3:0], R[3:0]}`，仅在 `rdn=0`（可见区域）时输出到 RGB 引脚，消隐区输出 0。

---

== ai_paddle.v — AI 对手

#h(2em)实现一个简单但有趣的 AI 对手来控制右挡板。

=== 追踪与死区

#h(2em)- 计算球的中心 Y 和挡板中心 Y
- 死区 `AI_DEAD_ZONE = 60px`：当球中心在挡板中心 ±60px 范围内时，AI 不主动追踪
- 死区外：球在上方则 `move_up`，球在下方则 `move_down`

=== 随机延迟

#h(2em)- 自由运行计数器 `rand_cnt` 不断递增
...
#h(2em)当球远离或球在死区内时，AI 进行小幅上下振荡（`osc_phase` 每32帧翻转方向），模拟"活"的表现而不会呆立不动。

=== 回中行为

#h(2em)当球远离 AI 时，挡板缓慢回到屏幕中心（Y=240），为下次防守做准备。

---

== ps2_keyboard.v — PS/2 键盘接口

#h(2em)实现 PS/2 协议的接收端，解析键盘扫描码并映射为游戏控制信号。

=== PS/2 协议基础

#h(2em)PS/2 协议是 11 位异步串行协议：
- 1 位起始位（0）
- 8 位数据位（LSB first）
- 1 位奇校验位
- 1 位停止位（1）
...
#h(2em)使用三级同步器 `ps2_clk_s0 → s1 → s2`：
- 第一级：消除亚稳态
...
== keypad_scanner.v — 矩阵键盘扫描器

#h(2em)实现 5×4 矩阵键盘的逐行扫描和去抖动。

=== 工作原理

#h(2em)矩阵键盘有5行4列共20个交叉点：
...
#h(2em)- 扫描周期：每行持续约 1ms（`SCAN_DELAY = 5035` 个 25.175MHz 时钟周期）
...
#h(2em)机械按键在按下/释放时会产生弹性抖动（约5~20ms），需要滤除。
...
== input_merger.v — 输入合并器

#h(2em)这是一个纯组合逻辑模块，使用 OR 运算将矩阵键盘和 PS/2 键盘的信号合并：
...
== buzzer_ctrl.v — 蜂鸣器音效控制

#h(2em)驱动无源蜂鸣器产生三种不同频率的方波音效。
...
#h(2em)- `half_period_cnt`：从0计数到 `current_half - 1`
- 计满时 `note_phase` 翻转 → 产生方波
- 方波频率 = 25.175MHz / (2 × current_half)

=== 边沿检测

#h(2em)游戏逻辑输出的脉冲较宽（约16ms），需要通过延迟一拍进行上升沿检测，将其转换为单时钟周期的窄脉冲，避免重复触发。

=== 优先级

#h(2em)若多个事件同时到来，游戏结束 > 得分 > 击球，后者覆盖前者的音高设定。

---

== led_status.v — LED 状态指示

#h(2em)8 个 LED 指示灯映射如下：
...
== seg_display.v — 七段数码管显示

#h(2em)4 位共阳极七段数码管显示当前难度模式。

=== 动态扫描原理

#h(2em)4 位数码管共享 8 根段选线（a~g, dp），通过分时选通实现"视觉暂留"：
...
#h(2em)封装 Xilinx MMCM（混合模式时钟管理器）IP 核 `clk_wiz_0`：
- 输入：100MHz 系统时钟
...
== powerup_ctrl.v — 道具控制器

#h(2em)实现"加宽挡板"道具的生成、显示和碰撞检测。
...
#h(2em)- 基础冷却：`COOLDOWN_BASE = 480` 个 game_tick ≈ 8 秒
- 随机附加：`rand_cnt[5:0]`（0~63），总冷却 ≈ 8~9 秒

=== 道具生成

#h(2em)- *X 位置*：交替出现在左/右挡板区域的中线处
...
#h(2em)道具为 6×6 像素，检测其与挡板的 Y 范围重叠：
```
...
#h(2em)`LIFETIME = 150` 个 game_tick ≈ 2.5 秒，超时后消失进入冷却。

---

= 设计总结

#h(2em)本项目以状态机为核心驱动游戏流程，以 game_tick 机制将物理速率与系统时钟解耦，以逐像素渲染方式生成 VGA 画面。各模块职责清晰、接口简洁，体现了"分而治之"的数字系统设计思想：
