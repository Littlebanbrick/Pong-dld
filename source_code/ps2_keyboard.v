    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (is_extended) begin
                case (key_make_code)
                    8'h75: right_up   <= !is_break;   // Up arrow
                    8'h72: right_down <= !is_break;   // Down arrow
                    8'h6B: left_up    <= !is_break;   // Left arrow (we use as left_up? No, we want W/S for left)
                    // We'll map left_up/down to W/S (not extended)
                    // For arrows, we just map to right paddle
                endcase
            end else begin
                case (key_make_code)
                    8'h1D: left_up    <= !is_break;   // W
                    8'h1B: left_down  <= !is_break;   // S
                    8'h5A: start_pause <= !is_break;  // Enter
                endcase
            end
